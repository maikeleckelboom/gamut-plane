...
function validateColorFunction(body: string): void {
  ...
  if (!space || !["srgb", "display-p3", "rec2020"].includes(space)) {
    fail("color() supports only srgb, display-p3, and rec2020 inputs");
  }
...
/** Strict CSS color adapter. All successful results enter the engine as OKLCH. */
export function parseUserColor(raw: string): ChromavertColor {
  ...
  try {
    ...
    const color: ChromavertColor = {
      ...
      source: { kind: "input", raw: raw.trim() },
    };
