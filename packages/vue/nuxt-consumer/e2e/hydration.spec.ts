...
for (const path of ["/", "/prerendered", "/?narrow=1"]) {
  test(`server document hydrates in place and edits both views: ${path}`, async ({
    ...
    test.skip(
      process.env.FIXTURE_MODE === "generated" && path.includes("?"),
      "Generated HTML has build-time state; narrow layout is also checked after resize.",
    );
    ...
    await expect(page.locator("[data-events]")).toHaveText('{"changes":0,"commits":0,"cancels":0}');
...
test("hidden reveal, resize and route remount keep lifecycle silent", async ({ page }) => {
  const errors = diagnostics(page);
  await page.goto(process.env.FIXTURE_MODE === "generated" ? "/" : "/?hidden=1");
  await expect(page.locator('[data-render-color-space="pending"]')).toHaveCount(0);
  if (process.env.FIXTURE_MODE === "generated")
...
for (const capability of ["srgb", "unavailable"]) {
  test(`Canvas ${capability} preserves hydrated controls and authored values`, async ({ page }) => {
    ...
    await expect(page.locator("[data-events]")).toHaveText('{"changes":0,"commits":0,"cancels":0}');
    const host = page.locator('[data-host="oklch"]');
...
test("independent requests preserve different authored values", async ({ browser }) => {
  test.skip(
    process.env.FIXTURE_MODE === "generated",
    "Static documents have build-time state; request isolation is verified on both SSR servers.",
  );
  ...
  try {
    ...
    for (const page of pages)
      await expect(page.locator('[data-render-color-space="pending"]')).toHaveCount(0);
    const first = await color(pages[0]!.locator('[data-host="oklch"]'));
    const second = await color(pages[1]!.locator('[data-host="oklch"]'));
